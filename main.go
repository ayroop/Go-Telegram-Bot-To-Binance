...
func main() {
    ...
    csrfMiddleware := csrf.Protect(csrfKey, csrf.Secure(true)) // Set csrf.Secure(true) if using HTTPS

    // Initialize the Telegram bot if the configuration is set
    if GlobalConfig.TelegramBotToken != "" && GlobalConfig.TelegramChatID != 0 {
        if err := initTelegramBot(&GlobalConfig); err != nil {
    ...
    http.Handle("/admin/login", csrfMiddleware(http.HandlerFunc(adminLoginHandler)))
    http.Handle("/admin/config", csrfMiddleware(http.HandlerFunc(adminConfigHandler)))

    // Webhook handler
    http.HandleFunc("/webhook", webhookHandler)

    // Optionally, redirect root URL to admin login
